(define (domain cleanup)
    ...
    (:predicates
        ...
        (upright ?container - container) ; whether the container is upright.
    )
    ...
    (:action open-drawer
     ...
     :precondition (and (not (open ?drawer)) (upright ?drawer) (free ?gripper))
     :effect (and
    ...
    (:action close-drawer
     :parameters (?drawer - drawer ?gripper - gripper)
     :precondition (and (open ?drawer) (upright ?drawer) (free ?gripper))
     :effect (and
        (exclusively-occupying-gripper ?drawer ?gripper)
        (not (free ?gripper))
        (not (open ?drawer)))
    ...
    (:action place-in-drawer-from-gripper
     ...
     :precondition (and (open ?drawer) (upright ?drawer) (small-enough-to-fit-in-container ?tabletop-object ?drawer) (large-enough-for-gripper-to-reach-inside ?drawer ?gripper) (exclusively-occupying-gripper ?tabletop-object ?gripper) (not (free ?gripper)))
     :effect (and
