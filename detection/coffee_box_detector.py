...
from mimicgen.envs.robosuite.coffee import Coffee_Box_Novelty

class CoffeeBoxDetector(Detector):
    def __init__(self, env, return_int=False):
        ...
        self.object_types = {
            'tabletop_object':['coffee-pod', 'coffee-machine-lid', 'coffee-pod-holder', 'mug', 'box'], 
            'container':['coffee-pod-holder', 'box', 'mug'], 'gripper':['gripper'], 
            'mug':['mug'], 
        ...
        self.grounded_object_to_pddl_object = {'mug':'mug1', 'coffee-machine-lid':'lid1', 'coffee-pod-holder':'coffee-pod-holder1', 'box':'box1', 'coffee-pod':'coffee-pod1', 'table':'table1', 'gripper':'gripper1'}

        self.grounded_tabletop_object_to_coffee_class_object = {'mug':self.env.mug, 'coffee-machine-lid':self.env.coffee_machine_lid, 'coffee-pod-holder':self.env.coffee_pod_holder, 'box':self.env.box, 'coffee-pod':self.env.coffee_pod, 'gripper':self.env.robots[0].gripper}
    ...
    def inside(self, tabletop_obj:str, container:str) -> bool:
        ...
        if container == 'coffee-pod-holder':
            ...
            if tabletop_obj in ('mug', 'coffee-pod'):
                return self.env.check_in_box(tabletop_obj)
            else:
                return False # only mugs and coffee pods can be inside the box 
    ...
    def open(self, container:str) -> bool:
        ...
        elif self._is_type(container, 'box'):
            return True # box is always open
    ...
    def verify_env(self, env) -> bool:
        ...
        while hasattr(env, 'env'):
            env = env.env
            if isinstance(env, Coffee_Box_Novelty):
                return True
        return False
