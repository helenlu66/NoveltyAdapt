...
class OpenCoffeePodHolder(Executor):
    ...
    def execute(self, detector:Detector, grounded_operator:fs.Action=None, render=False):
        """execute the open coffee pod holder operator in the coffee
        ...
        if binary_obs["open coffee-pod-holder1"] and binary_obs["free gripper1"]:
            return True
        ...
        while steps < 2:
            ...
            steps += 1
        binary_obs = detector.detect_binary_states(env)
        success=  binary_obs["open coffee-pod-holder1"] and binary_obs["free gripper1"]
        print(f"open coffee pod holder successful: {success}")
        return success


COFFEE_BOX_EXECUTORS = {
    ...
    "open-coffee-pod-holder": OpenCoffeePodHolder(), # dirty hack to avoid implementing open-coffee-pod-holder
    "free-gripper-from-object-and-return-to-neutral": OpenCoffeePodHolder(), # dirty hack to avoid implementing free-gripper-from-large-object
}

if __name__ == "__main__":
    # Test the coffee executor

    # make the coffee box novelty env
    domain = "coffee_box"
    config = load_config("config.yaml")
    env = load_env(domain, config['simulation'])
    env = VisualizationWrapper(env, indicator_configs=None)
    env.viewer.set_camera(camera_id=0)
    executor = COFFEE_BOX_EXECUTORS["open-coffee-pod-holder"]
    detector = CoffeeBoxDetector(env)
    success = executor.execute(detector, None, render=True)
    print(f"Execution of open-coffee-pod-holder successful: {success}")
    env.render()
    for i in range(50):
