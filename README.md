...
### Training LLM Candidates
Once you attach to the docker container, you can start a tmux window
```bash
tmux new -s {domain_llm_operator_seednum}
```

Once inside the tmux session, you can
```bash
python train_llm_candidates.py --domain {domain} --seed {seednum}
```
